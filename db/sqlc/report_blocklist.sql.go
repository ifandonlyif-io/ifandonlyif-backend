...
const checkBlocklists = `-- name: CheckBlocklists :one
SELECT 
    CASE 
...
func (q *Queries) CheckBlocklists(ctx context.Context, id uuid.UUID) (CheckBlocklistsRow, error) {
	row := q.db.QueryRowContext(ctx, checkBlocklists, id)
	var i CheckBlocklistsRow
	err := row.Scan(&i.ReviewStatus, &i.TimeStamp)
	return i, err
}

const checkExistBlocklists = `-- name: CheckExistBlocklists :one
...
func (q *Queries) CheckExistBlocklists(ctx context.Context, httpAddress string) (uuid.UUID, error) {
	row := q.db.QueryRowContext(ctx, checkExistBlocklists, httpAddress)
	var id uuid.UUID
	err := row.Scan(&id)
	return id, err
}
